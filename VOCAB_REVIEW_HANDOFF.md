...
- `MT28 Paper 4`
- Source tag: `mock-unseen-mt28-paper4-reviewed`
- Added 22 reviewed entries, including Meatless Monday / meatless / environmental footprint / meat farming / take on a challenge / picky eater / stock / swap out / gift basket / give out / tamale / cornmeal / mole / corn husk
- Verified with `npm test`, `npm run check:js`, `npm run build:web`, and Battleship sync / dry-run.

Next likely item:

- Paper 3 is complete. `MT52`, `MT74`, `MT76`, and `MT77` have no Paper 4 file in the current download set, and `MT7` / `MT8` / `MT9` / `MT10` / `MT11` / `MT12` / `MT13` / `MT14` / `MT16` / `MT18` / `MT19` / `MT21` / `MT23` / `MT24` / `MT26` / `MT28` / `MT56` / `MT57` / `MT59` / `MT60` / `MT62` / `MT63` / `MT64` / `MT65` / `MT66` / `MT67` / `MT68` / `MT69` / `MT70` / `MT71` / `MT72` / `MT73` / `MT75` / `MT78` / `MT79` / `MT81` / `MT83` / `MT84` / `MT85` / `MT86` / `MT87 Paper 4` are complete. Continue with the next available missing Paper 4 item, or continue remaining Paper 1 items from the review log.
