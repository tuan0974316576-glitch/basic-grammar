...
// ★★★ RANKING SCREEN FUNCTIONS ★★★
let currentRankingTab = 'total-xp'; // Default tab
...
async function loadTotalXPRanking() {
    ...
    try {
        ...
        // Convert to array and sort by totalXP
        ...
        top100.forEach((user, index) => {
            ...
            // ★★★ Dynamic Icon Color based on global rank ★★★
        ...
        // ★★★ Trigger staggered fadeIn animation ★★★
...
async function loadPVPWinRateRanking() {
    ...
    try {
        ...
        // Convert to array - no minimum wins requirement
        const userArray = Object.keys(users)
            ...
            .filter(user => user.pvpWins > 0); // Only users with at least 1 win
...
// 0. 取消 AUTHENTICATING 遮罩 (安全網：防止 signInWithPopup 唔觸發 catch)
let authOverlayTimeout = null;
window.cancelAuthOverlay = function() {
    const overlay = document.getElementById('login-overlay');
    if(overlay) overlay.style.display = 'none';
    if(authOverlayTimeout) { clearTimeout(authOverlayTimeout); authOverlayTimeout = null; }
    if(typeof playSound === 'function') playSound('delete-sfx');
}
function startAuthOverlayTimeout() {
    ...
    }, 30000); // 30 秒後自動關閉（延長至30秒以確保 Firebase 請求完成）
}
...
window.loginWithGoogle = function() {
    ...
    .then((result) => {
        // 登入成功！onAuthStateChanged 會自動處理轉場
        // ★★★ 不要清除 timeout，讓 overlay 保持顯示直到 onAuthStateChanged 完成 ★★★
...
function updateHUD(name) {
    ...
    if (typeof getRankForXP === 'function') {
        ...
        if (rankEl) {
            // ★★★ Task 4: Check global rank and add colored icon ★★★
...
// ★★★ Task 4: Real-time Dual Rank Check for Main Menu HUD ★★★
async function checkGlobalRankAndUpdateIcon(rankEl, rank) {
    ...
    // First set default display with white icon (smaller and closer)
    ...
    // ★★★ CRITICAL FIX: Don't query Firebase if player ID is not ready yet ★★★
    ...
    try {
        ...
        // ★★★ Check 1: Total XP Ranking (Top 10) ★★★
        const usersRef = ref(window.db, 'users');
        ...
        if (snapshot.exists()) {
            ...
            console.log('[HUD] Total XP Top 10:', top10.map(u => `${u.id.substring(0,6)}:${u.xp}`));
            console.log('[HUD] My XP rank index:', xpRankIndex);
        }

        // ★★★ Check 2: PVP Wins Ranking (Top 10 by total wins) ★★★
        let pvpRankIndex = -1;
        ...
        console.log(`[HUD] ✅ Final result - XP rank: ${xpRankIndex !== -1 ? '#' + (xpRankIndex + 1) : 'unranked'} (${xpColor}) | PVP rank: ${pvpRankIndex !== -1 ? '#' + (pvpRankIndex + 1) : 'unranked'} (${pvpColor}) | Best: ${bestColor}`);
        console.log(`[HUD] Icon path: ${iconPath}`);
    } catch (error) {
        console.error('[HUD] ❌ Error checking ranks:', error);
        // Keep the default white icon that was already set
    }
}

// ★★★ Task 5: Update Opponent Rank Badge in PVP Battle ★★★
async function updateOpponentRankBadge(opponentId) {
    try {
        ...
        // Query top 10 players to check opponent's global rank
        const usersRef = ref(window.db, 'users');
        const top10Query = query(usersRef, orderByChild('xp'), limitToLast(10));
        const snapshot = await get(top10Query);

        if (!snapshot.exists()) {
            // No leaderboard data, use white icon
        ...
        if (opponentIndex !== -1) {
            // Opponent is in top 10, determine color
...
function updateExpBar(currentXP, rank) {
    ...
    if (nextRankXP === null) {
        ...
        // Update label to show next rank name
        const nextRankData = RANK_TABLE[rank.level]; // rank.level is current level (0-indexed), so +1 is next
...
// ★★★ UPDATE SUPPLIES DISPLAY FUNCTION ★★★
...
function showMainMenu() {
    // ★★★ CRITICAL FIX: Hide login overlay ★★★
    const overlay = document.getElementById('login-overlay');
    if (overlay) {
        overlay.style.display = 'none';
...
function switchHudPanel(panelId) {
    ...
    if(target) {
        ...
        // ★★★ Special handling for user-profile-panel to trigger holoAppear animation ★★★
