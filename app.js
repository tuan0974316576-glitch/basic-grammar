...
const VOCAB_EXAMPLES_CACHE_KEY = "basic_vocab_examples_cache_v2";
...
function renderTenseQuestion(question) {
  if (!shouldAskTenseQuestion()) {
    askTenseBlank(question, `練習：${question.tenseLabel}。請打空格入面嘅動詞形式。`, { playCorrectSound: false });
    return;
  }
...
function askTenseBlank(question, message = "時態正確。請打空格入面嘅動詞形式。", options = {}) {
  setTextEntryValue(el.countableCorrectionInput, "");
  el.stepLabel.textContent = "English sentence";
  ...
  if (options.playCorrectSound !== false) {
    playUiSound("correct");
  }
}
