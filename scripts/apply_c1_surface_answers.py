...
C1_BANK = ROOT / "question_bank_c1.json"
...
def main():
    ...
    updates = [
        ...
        if item.get("cefr") == "C1" and item.get("bank") == "question_bank_c1.json"
    ]

    bank = json.loads(C1_BANK.read_text(encoding="utf-8"))
    ...
    C1_BANK.write_text(json.dumps(bank, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    for idx, entry in enumerate(sorted_entries):
        if entry.get("cefr") == "C1" and entry.get("word") in bank_lookup:
    ...
    vocab_text = replace_level_block(vocab_text, "L5", build_level_block(bank))
