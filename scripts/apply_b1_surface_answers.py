...
B1_BANK = ROOT / "question_bank_b1.json"
...
def main():
    ...
    b1_updates = [
        ...
        if item.get("cefr") == "B1" and item.get("bank") == "question_bank_b1.json"
    ]

    bank = json.loads(B1_BANK.read_text(encoding="utf-8"))
    by_word = {entry["word"]: entry for entry in bank}

    changes = 0
    for item in b1_updates:
    ...
    B1_BANK.write_text(json.dumps(bank, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    for idx, entry in enumerate(sorted_entries):
        if entry.get("cefr") == "B1" and entry.get("word") in bank_lookup:
    ...
    vocab_text = replace_level_block(vocab_text, "L3", build_level_block(bank))
