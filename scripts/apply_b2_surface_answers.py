...
B2_BANK = ROOT / "question_bank_b2.json"
...
def main():
    ...
    b2_updates = [
        ...
        if item.get("cefr") == "B2" and item.get("bank") == "question_bank_b2.json"
    ]

    bank = json.loads(B2_BANK.read_text(encoding="utf-8"))
    by_word = {entry["word"]: entry for entry in bank}

    changes = 0
    for item in b2_updates:
    ...
    B2_BANK.write_text(json.dumps(bank, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    for idx, entry in enumerate(sorted_entries):
        if entry.get("cefr") == "B2" and entry.get("word") in bank_lookup:
    ...
    vocab_text = replace_level_block(vocab_text, "L4", build_level_block(bank))
