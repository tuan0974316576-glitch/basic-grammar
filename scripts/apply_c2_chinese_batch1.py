from __future__ import annotations

import csv
import json
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]

TRANSLATIONS = {
    "a cappella": "無伴奏合唱",
    "abseil": "繩降",
    "accord": "協議／一致",
    "admonitory": "告誡的",
    "africanize": "使非洲化",
    "ala": "像／以…方式",
    "alfresco": "戶外的／在戶外",
    "aloof": "冷淡的／疏離的",
    "ambiguity": "歧義／含糊",
    "ambiguous": "模棱兩可的",
    "ambush": "埋伏／伏擊",
    "anaphylactic": "過敏性休克的",
    "anticipation": "預期／期待",
    "apiarist": "養蜂人",
    "apocalypse": "末日／大災難",
    "apoplectic": "暴怒的",
    "arboreal": "樹棲的",
    "arcane": "艱深難懂的／神秘的",
    "attest": "證明／證實",
    "auditorium": "禮堂／觀眾席",
    "authoritarian": "專制的／威權的",
    "autopsy": "驗屍／屍體解剖",
    "avert": "避免／防止",
    "awesome": "令人敬畏的／極好的",
    "banal": "陳腐的／平庸的",
    "bandwagon": "潮流／跟風",
    "banyan": "榕樹",
    "barb": "倒鉤／尖刻的話",
    "barge": "駁船／闖入",
    "bazseedling": "幼苗",
    "beehive": "蜂巢",
    "belie": "掩飾／與…不符",
    "benign": "良性的／和善的",
    "bereavement": "喪親之痛",
    "berserk": "狂暴的／發狂",
    "bicker": "爭吵／拌嘴",
    "bile": "膽汁／怒氣",
    "billow": "翻騰／巨浪",
    "blare": "刺耳地響",
    "bleak": "荒涼的／黯淡的",
    "bloke": "傢伙／男人",
    "boatload": "大量",
    "bodacious": "驚人的／大膽的",
    "botchers": "拙劣的工匠／把事情搞砸的人",
    "bracken": "蕨類植物",
    "brigade": "旅／隊伍",
    "bristle": "豎起／短硬毛",
    "bronze": "青銅／古銅色",
    "burrow": "地洞／挖洞",
    "buzz": "嗡嗡聲／熱議",
    "calamity": "災難／大禍",
    "calibrate": "校準",
    "callus": "老繭",
    "cannibalize": "拆用／同類相食",
    "canon": "準則／真作",
    "canopy": "樹冠／頂篷",
    "catastrophe": "災難",
    "celestial": "天體的／天空的",
    "chariot": "戰車／馬車",
    "chasten": "懲戒／使收斂",
    "cherub": "小天使",
    "chide": "責罵／斥責",
    "chime": "鐘聲／和諧一致",
    "chink": "裂縫／叮噹聲",
    "chuff": "噴氣聲／開心",
    "chum": "老友／魚餌碎塊",
    "churn": "攪動／翻騰",
    "clang": "鏗鏘聲",
    "claustrophobic": "幽閉恐懼的",
    "clincher": "關鍵論點／決定因素",
    "clog": "阻塞／木底鞋",
    "coerce": "脅迫／強迫",
    "collate": "整理／校對",
    "commotion": "騷動／喧鬧",
    "complimentary": "稱讚的／免費贈送的",
    "conduit": "管道／渠道",
    "confucian": "儒家的／儒家學者",
    "consortium": "財團／聯盟",
    "conspicuous": "顯眼的／顯著的",
    "contradictory": "矛盾的／對立的",
    "coral": "珊瑚",
    "counterfactual": "反事實的",
    "crackdown": "嚴厲打擊／鎮壓",
    "cram": "塞滿／死記硬背",
    "crane": "鶴／起重機／伸長脖子",
    "crank": "怪人／曲柄",
    "crave": "渴望",
    "credence": "可信性／信任",
    "credulously": "輕信地",
    "crimson": "深紅色",
    "crisp": "脆的／乾爽的",
    "critter": "小生物",
    "crucible": "熔爐／考驗",
    "crumble": "崩潰／碎裂",
    "culmination": "頂點／最終結果",
    "curbside": "路邊",
    "daft": "愚蠢的",
    "dazzle": "使目眩／使驚艷",
    "debunker": "揭穿真相的人",
    "deception": "欺騙／詭計",
    "decongest": "緩解擁擠",
    "deepen": "加深",
    "deface": "污損／損毀外觀",
    "defecate": "排便",
    "definitive": "明確的／權威性的",
    "deity": "神明",
    "delusion": "妄想／錯覺",
    "demolition": "拆毀／爆破",
    "deprave": "使墮落",
    "desist": "停止／中止",
    "desperation": "絕望／孤注一擲",
    "destine": "注定／預定",
    "destructibility": "可毀滅性",
    "developer": "開發者／發展商",
    "dharma": "佛法／正法",
    "dice": "骰子／切成丁",
    "dichotomy": "一分為二／對立",
    "dictatorial": "獨裁的／專橫的",
    "didactic": "說教的／教學的",
    "dim": "昏暗的／使變暗",
    "din": "喧鬧聲",
    "dioxide": "二氧化物",
    "disaffection": "不滿／疏離",
    "discernible": "可辨認的",
    "dismantle": "拆除／拆解",
    "disruptive": "破壞性的／擾亂性的",
    "dissociate": "使脫離／使分離",
    "dullard": "蠢人",
    "dystopia": "反烏托邦",
    "dystopian": "反烏托邦的",
    "eclectic": "兼收並蓄的",
    "elasticated": "鬆緊帶的",
    "elder": "長者／年長的",
    "elimination": "消除／淘汰",
    "embarrass": "使尷尬",
    "embellish": "潤飾／裝飾",
    "embolden": "使更大膽",
    "emit": "發出／排放",
    "enchant": "使著迷／施魔法",
    "enclave": "飛地／小圈子",
    "encode": "編碼",
    "endemic": "地方性的／特有的",
    "enliven": "使活躍／使生動",
    "entail": "需要／牽涉",
    "enthuse": "熱情談論／使熱衷",
    "entrails": "內臟",
    "equivocate": "含糊其詞",
    "ethiopian": "埃塞俄比亞的／埃塞俄比亞人",
    "excel": "擅長／優於",
    "excruciate": "使極度痛苦",
    "expressive": "表現力強的",
    "extol": "頌揚／讚美",
    "extrapolate": "推斷／外推",
    "extravagant": "奢侈的／過度的",
    "facade": "外牆／表面",
    "facial": "面部的／美容護理",
    "fad": "一時流行",
    "faddish": "趕時髦的",
    "fatuous": "愚蠢自滿的",
    "fauna": "動物群",
    "feckless": "軟弱無能的",
    "fecundity": "多產／豐饒",
    "feistiness": "好鬥／火爆",
    "fervently": "熱切地",
    "fervor": "熱忱／激情",
    "fetishist": "迷戀者／崇拜者",
    "fictional": "虛構的",
    "fiddle": "小提琴／擺弄",
    "fig": "無花果",
    "finch": "雀科小鳥",
    "fissure": "裂縫",
    "fittingly": "恰如其分地",
    "fixate": "使著迷／使固著",
    "flaunt": "炫耀",
    "flier": "傳單／飛行者",
    "flirt": "調情／輕率對待",
    "flora": "植物群",
    "flummox": "使困惑",
    "follower": "追隨者",
    "footloose": "無拘無束的",
    "forage": "覓食／搜尋",
    "forager": "採集者／覓食者",
    "forebear": "祖先／前人",
    "foreseeable": "可預見的",
    "fragrance": "香氣",
    "freewheel": "自由滑行／隨心所欲",
    "frenzy": "狂熱／瘋狂",
    "fresco": "濕壁畫",
    "fringe": "邊緣／流蘇",
    "frown": "皺眉",
    "fulfilment": "實現／滿足感",
    "fumble": "笨拙地摸索／失手",
    "fury": "狂怒",
    "futile": "徒勞的",
    "gaggle": "一群鵝／嘰嘰喳喳的一群人",
    "garishness": "俗艷",
    "gazillion": "無數／天文數字",
    "glaze": "釉／上釉",
    "glider": "滑翔機",
    "glisten": "閃耀／發亮",
}


def main() -> None:
    bank_path = ROOT / "question_bank_c2.json"
    missing_path = ROOT / "question_bank_c2_missing_chinese.csv"
    translations_path = ROOT / "c2_ch_translations.json"

    with bank_path.open("r", encoding="utf-8") as f:
        bank = json.load(f)

    for entry in bank:
        if entry["word"] in TRANSLATIONS:
            entry["ch"] = TRANSLATIONS[entry["word"]]

    with bank_path.open("w", encoding="utf-8") as f:
        json.dump(bank, f, ensure_ascii=False, indent=2)
        f.write("\n")

    rows = [
        {"word": entry["word"], "cefr": entry.get("cefr", "")}
        for entry in bank
        if not (entry.get("ch") or "").strip()
    ]
    with missing_path.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["word", "cefr"])
        writer.writeheader()
        writer.writerows(rows)

    existing = {}
    if translations_path.exists():
        existing = json.loads(translations_path.read_text(encoding="utf-8"))
    existing.update(TRANSLATIONS)
    translations_path.write_text(
        json.dumps(existing, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )


if __name__ == "__main__":
    main()
