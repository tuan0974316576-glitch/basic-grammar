...
- Use `npm run vocab:review-index -- --out private_exports/teacher_vocab_review_index.json` after creating review batches. The private index tracks generated batch files, current coverage, and the next offset so the 4000-word cleanup can continue in order.
- Use `npm run vocab:review-next` to generate the next high-value teacher-audit batch automatically from the private index. It writes JSON/CSV/XLSX and refreshes the index.
