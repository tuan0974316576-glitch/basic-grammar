...
- To fold same-day cloud entries back into the reviewed local bank, run `npm run vocab:export-teacher-live` to export Firestore `teacherVocabLive` into `private_exports/teacher_live_vocab_snapshot.json/csv`, then run `npm run vocab:review-next:teacher-live -- --count 1`. This creates private `teacher_live_vocab_review_batch_*` files from the cloud snapshot so teacher-live entries can be checked, cleaned, and promoted through the same preflight / promote-plan / apply-plan flow as other sources.
