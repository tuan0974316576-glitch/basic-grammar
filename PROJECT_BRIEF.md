...
- Student-facing vocab meaning lookup must stay deterministic and fast: teacher bank -> curated local supplement -> CC-CEDICT supplement -> `待老師確認`. Do not call live Azure / Gemini meaning fallback while students type.
...
- Student-facing meaning lookup order is: `teacher_vocab_bank.js` / Austin Sir notes first, then `vocab_sense_bank.js`, then `cc_cedict_supplement.js`, then a selectable `待老師確認` entry. Do not guess from generic dictionaries in the student UI.
- `vocab_sense_bank.js` is the curated local multi-meaning supplement for common words that generic dictionaries often confuse. It sits after teacher notes and before the CC-CEDICT supplement. Saved vocab, example lookup, and revision should follow the selected sense (`word + POS/type + meaning`), so a word such as `game` only shows `野味` examples if the student selected `n. 野味`.
...
- If teacher bank, curated supplement, and CC-CEDICT supplement all have no match, mark it as `待老師確認`. Do not call Firebase / Azure / Gemini to invent a student-facing meaning.
- When a student saves a `待老師確認` meaning, queue it locally and sync it after login to `users/{uid}/vocabMeaningReviews/{wordId}` with status `pending`. This gives the teacher a clean review list without blocking the student or guessing a meaning.
