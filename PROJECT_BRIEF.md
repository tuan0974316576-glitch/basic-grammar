...
- Student-facing vocab meaning lookup must stay deterministic and fast: teacher bank -> curated local supplement -> CC-CEDICT supplement. Do not show placeholder meanings or call live Azure / Gemini meaning fallback while students type.
...
- Student-facing meaning lookup order is: `teacher_vocab_bank.js` / Austin Sir notes first, then `vocab_sense_bank.js`, then `cc_cedict_supplement.js`. Do not guess from generic dictionaries in the student UI, and do not show placeholders as selectable meanings.
...
- If teacher bank, curated supplement, and CC-CEDICT supplement all have no match, show no selectable meaning and keep the Add button disabled. Fix coverage by improving the teacher / curated vocab bank, not by asking students to save an unknown placeholder.
